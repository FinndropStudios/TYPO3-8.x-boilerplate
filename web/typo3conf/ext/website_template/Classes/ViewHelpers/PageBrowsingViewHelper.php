<?php
namespace Finndrop\WebsiteTemplate\ViewHelpers;
...
class PageBrowsingViewHelper extends AbstractViewHelper implements CompilableInterface
{
    ...
    public static function renderStatic(array $arguments, \Closure $renderChildrenClosure, RenderingContextInterface $renderingContext)
    {
        ...
        $content = '';
        $content .= '<div class="row paginator">';

        // prev page
        // show on all pages after the 1st one
        $content .= '<div class="col-xs-3 prev">';
        $label = LocalizationUtility::translate('displayResults.previous', 'IndexedSearch');
        if ($currentPage > 0) {
            $content .= self::makecurrentPageSelector_link($label, $currentPage - 1, $freeIndexUid, true);
        } else {
            $label = self::makecurrentPageSelector_link($label, $currentPage - 1, $freeIndexUid, false);
            $content .= '<span class="inactive">' . $label . '</span>';
        }
        $content .= '</div>';
        $content .= '<div class="col-xs-6 centered">';
        ...
        for ($a = $minPage; $a <= $maxPage; $a++) {
            ...
            if ($a === $currentPage) {
                $label = self::makecurrentPageSelector_link($label, $a, $freeIndexUid, false);
                $content .= '<span class="current">' . $label . '</span>';
            } else {
                $label = self::makecurrentPageSelector_link($label, $a, $freeIndexUid, true);
                $content .= $label;
            }
        }
        $content .= '</div>';
        $content .= '<div class="col-xs-3 rightened next">';
        // next link
     ...
     * @param boolean $link link or string
     ...
     */
    protected static function makecurrentPageSelector_link($str, $p, $freeIndexUid, $link)
    {
        if($link == true) {
